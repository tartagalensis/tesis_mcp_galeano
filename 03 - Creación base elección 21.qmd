---
title: "03 - Creación base eleccion 2021"
...
elecciones_21 <- read_csv("DipNacPBAxcircuito_2011-21.csv") %>% 
  filter(id_eleccion == "dip_gral2021")


votos_positivos <- elecciones_21 %>% group_by(DPTO, circuito) %>% summarise(votos_positivos =  sum(votos_partido_politico))

votos_lla <- elecciones_21 %>% 
  filter(id_fuerza_politica == '503') %>% 
...
votos_jxc <- elecciones_21 %>% 
  filter(id_fuerza_politica == '506') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_jxc = round((votos_partido_politico/votos_positivos)*100, digits = 3)) %>% 
  select(DPTO, circuito, voto_jxc) 

votos_2021 <- elecciones_21 %>% 
  filter(id_fuerza_politica == '507') %>% 
  left_join(votos_positivos) %>% 
...
ggsave("plots/voto_pobreza_2021.png", mi_grafico, width = 10, height = 6, dpi = 300)
```
