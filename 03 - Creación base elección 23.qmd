---
title: "03 - Creación base eleccion 2023"
...
elecciones_23 <- read_csv("DipNacPBAxcircuito_2011-23.csv") %>% 
  filter(id_eleccion == "dip_gral2023")


votos_positivos <- elecciones_23 %>% group_by(DPTO, circuito) %>% summarise(votos_positivos =  sum(votos_partido_politico))

votos_lla <- elecciones_23 %>% 
  filter(id_fuerza_politica == '20135') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_lla = round((votos_partido_politico/votos_positivos)*100, digits = 3)) %>% 
  select(DPTO, circuito, voto_lla) 

votos_jxc <- elecciones_23 %>% 
  filter(id_fuerza_politica == '20132') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_jxc = round((votos_partido_politico/votos_positivos)*100, digits = 3)) %>% 
  select(DPTO, circuito, voto_jxc) 

votos_2023 <- elecciones_23 %>% 
  filter(id_fuerza_politica == '20134') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_fdt = round((votos_partido_politico/votos_positivos)*100, digits = 3)) %>% 
  select(DPTO, circuito, blancos, nulos, votos_positivos, voto_fdt, electores) %>% 
  left_join(votos_lla) %>% 
  left_join(votos_jxc) %>% 
  print()
...
```{r}
vi_01 <- read_csv("data_socioeco/stats_circuitos/ingresos_pobreza_circuitos_2021.csv") %>% 
...
vi_02 <- read_csv("data_socioeco/stats_circuitos/Grupo_Etario_INDEC_circuitos_2021.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_03 <-read_csv("data_socioeco/stats_circuitos/CAT_OCUP_circuitos_2021.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>%
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_04 <- read_csv("data_socioeco/stats_circuitos/P0910_circuitos_2021.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_05 <- read_csv("data_socioeco/stats_circuitos/URP_circuitos_2021.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_06 <-read_csv("data_politica/Intendentes PBA 2019 - 2023.csv") %>% 
  ...
  mutate(dummy_fdt = if_else(partido == 'FdT', 1, 0),
...
```{r}
past_year <- read_csv("data_socioeco/stats_circuitos/ingresos_pobreza_circuitos_2019.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
...
vi_08 <- read_csv("data_socioeco/stats_circuitos/PP07K_circuitos_2021.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO),
...
base_fdt_v01 <- votos_2023 %>% 
  ...
  left_join(vi_07) %>%
  left_join(vi_08) %>% 
  select(DPTO, circuito, voto_fdt, electores, everything()) %>% 
...
modelo <- glm(voto_fdt ~  pobreza + indigencia + share_ed_uni + mayor_65 + share_ocup_3 +  share_informal + 
...
```{r}
modelo <- glm(voto_lla ~  pobreza + indigencia + share_ed_uni + mayor_65 + share_ocup_3 +  share_informal + 
      dummy_fdt +  log(electores) ,data = base_fdt_v01)
summary(modelo)
...
```{r}
modelo <- glm(voto_jxc ~  pobreza + indigencia + share_ed_uni + mayor_65 + share_ocup_3 +  share_informal + 
      dummy_fdt +  log(electores) ,data = base_fdt_v01)
summary(modelo)
...
datos_long <- base_fdt_v01 %>%
  select(voto_fdt, voto_jxc, voto_lla, pobreza) %>% 
  gather(key = "partido", value = "votos", voto_fdt, voto_jxc, voto_lla)

# Crear los gráficos
# Definir colores para cada partido
colores_partidos <- c("voto_fdt" = "lightblue", "voto_jxc" = "gold", "voto_lla" = "purple")
...
ggsave("plots/voto_pobreza_2023.png", mi_grafico, width = 10, height = 6, dpi = 300)
```
