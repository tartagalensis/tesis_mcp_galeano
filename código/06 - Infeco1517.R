...
data1<-read.csv("base_infeco_1517.csv")


dsPpre <- data1[,6:11] #De que col a que col van los rtados de las elecciones prim vuelta
dsPpre <- dsPpre/data1[,5] #n de col de electores
dsPpre <- cbind(dsPpre,1-rowSums(dsPpre))
dsPpre <- as.matrix(dsPpre)

dsGpre <- data1[,12:17] #de que col a que col van los rtados de las elecciones 2da vuelta
dsGpre <- dsGpre/data1[,5] #n de col de electores
dsGpre <- cbind(dsGpre,1-rowSums(dsGpre))
dsGpre<- as.matrix(dsGpre)

#Paso a General
newdata<-as.matrix(cbind(dsPpre,dsGpre))
estsPG<-params.estim(newdata,nR=7,nC=7) #10 filas (1 para blancos/ausentes), 3 cols (1 para blancos/ausentes)
fracsPG<-calc.fractions(estsPG,nR=7,nC=7)
colnames(fracsPG)<-colnames(dsGpre)
rownames(fracsPG)<-colnames(dsPpre)

##Datos Summary
VotosPaso <- data1[,6:11]
VotosPaso <- cbind(VotosPaso,data1[,5]-rowSums(VotosPaso))
VotosGeneral <- data1[,12:17]
VotosGeneral <- cbind(VotosGeneral ,data1[,5]-rowSums(VotosGeneral))
...
indice <- tibble(indice = c("FpV", "Progres",
                            "FIT", "JxC",
                            "UNA","Blancos","No votó"))
...
tabla %>%
  ...
  write_csv("inferenciaEcoResultados_1517.csv")
