...
data1<-read.csv("base_infeco_1921.csv")


dsPpre <- data1[,5:9] #De que col a que col van los rtados de las elecciones prim vuelta
dsPpre <- dsPpre/data1[,10] #n de col de electores
dsPpre <- cbind(dsPpre,1-rowSums(dsPpre))
dsPpre <- as.matrix(dsPpre)

dsGpre <- data1[,11:17] #de que col a que col van los rtados de las elecciones 2da vuelta
dsGpre <- dsGpre/data1[,10] #n de col de electores
dsGpre <- cbind(dsGpre,1-rowSums(dsGpre))
dsGpre<- as.matrix(dsGpre)

#Paso a General
newdata<-as.matrix(cbind(dsPpre,dsGpre))
estsPG<-params.estim(newdata,nR=6,nC=8) #10 filas (1 para blancos/ausentes), 3 cols (1 para blancos/ausentes)
fracsPG<-calc.fractions(estsPG,nR=6,nC=8)
colnames(fracsPG)<-colnames(dsGpre)
rownames(fracsPG)<-colnames(dsPpre)

##Datos Summary
VotosPaso <- data1[,5:9]
VotosPaso <- cbind(VotosPaso,data1[,10]-rowSums(VotosPaso))
VotosGeneral <- data1[,11:17]
VotosGeneral <- cbind(VotosGeneral ,data1[,10]-rowSums(VotosGeneral))
...
tabla %>%
  ...
  write_csv("inferenciaEcoResultados_1921.csv")
