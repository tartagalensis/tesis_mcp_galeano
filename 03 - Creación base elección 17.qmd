---
title: "03 - Creación base eleccion 2017"
...
elecciones_17 <- read_csv("DipNacPBAxcircuito_2011-21.csv") %>% 
  filter(id_eleccion == "dip_gral2017")


votos_positivos <- elecciones_17 %>% group_by(DPTO, circuito) %>% summarise(votos_positivos =  sum(votos_partido_politico))



votos_una <- elecciones_17 %>% 
  filter(id_fuerza_politica == '0511') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_massa = round((votos_partido_politico/votos_positivos)*100, digits = 1)) %>% 
  select(DPTO, circuito, voto_massa) 

votos_jxc <- elecciones_17 %>% 
  filter(id_fuerza_politica == '0518') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_jxc = round((votos_partido_politico/votos_positivos)*100, digits = 1)) %>% 
  select(DPTO, circuito, voto_jxc) 

votos_randazzo <- elecciones_17 %>% 
  filter(id_fuerza_politica == '0546') %>% 
  left_join(votos_positivos) %>% 
  mutate(voto_randazzo = round((votos_partido_politico/votos_positivos)*100, digits = 1)) %>% 
  select(DPTO, circuito, voto_randazzo) 

votos_2017 <- elecciones_17 %>% 
  filter(id_fuerza_politica == '0574') %>% 
  left_join(votos_positivos) %>% 
...
```{r}
vi_01 <- read_csv("data_socioeco/stats_circuitos/ingresos_pobreza_circuitos_2017.csv") %>% 
...
vi_02 <- read_csv("data_socioeco/stats_circuitos/Grupo_Etario_INDEC_circuitos_2017.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_03 <-read_csv("data_socioeco/stats_circuitos/CAT_OCUP_circuitos_2017.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>%
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
  mutate(pop_ocup = sum(`0.0`, `2.0`,`3.0`, na.rm = T)) %>% 
  select(DPTO, circuito, ocup_0 =`0.0`, ocup_2 =`2.0`, ocup_3 =`3.0`, pop_ocup) %>% 
...
```{r}
vi_04 <- read_csv("data_socioeco/stats_circuitos/P0910_circuitos_2017.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
```{r}
vi_05 <- read_csv("data_socioeco/stats_circuitos/URP_circuitos_2017.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO)) %>% 
  rowwise() %>% 
...
vi_06 <-read_csv("data_politica/Intendentes PBA 2011 - 2023.csv") %>% 
  ...
  mutate(dummy_fdt = if_else(partido == 'FpV', 1, 0),
         dummy_jxc = if_else(partido == 'JxC', 1, 0),
         dummy_massa = if_else(partido == "UNA", 1, 0)) %>% 
...
vi_08 <- read_csv("data_socioeco/stats_circuitos/PP07K_circuitos_2017.csv") %>% 
  filter(NOMPROV == "Buenos Aires") %>% 
  mutate(DPTO = as.character(DPTO),
...
```{r}
modelo_2 <- glm(voto_randazzo ~  pobreza + indigencia + share_ed_uni + mayor_65 + share_ocup_3 +  share_informal +
      dummy_fdt  + log(electores) ,data = base_fdt_v01)
summary(modelo_2)
...
```{r}
modelo_3 <- glm(voto_massa ~  pobreza + indigencia + share_ed_uni + mayor_65 + share_ocup_3 +  share_informal +
      dummy_fdt  + log(electores) ,data = base_fdt_v01)
summary(modelo_3)

with(summary(modelo_3), 1 - deviance/null.deviance)
```

```{r}
# Reestructurar los datos para facilitar el gráfico
datos_long <- base_fdt_v01 %>%
  select(voto_uc = voto_fpv, voto_jxc, voto_massa, voto_randazzo, pobreza) %>% 
  gather(key = "partido", value = "votos", voto_uc, voto_massa, voto_randazzo, voto_jxc)

# Crear los gráficos
# Definir colores para cada partido
colores_partidos <- c("voto_uc" = "lightblue", "voto_jxc" = "gold", "voto_massa" = "darkblue", voto_randazzo = "lightgreen")
...
ggsave("plots/voto_pobreza_2017.png", mi_grafico, width = 10, height = 6, dpi = 300)
```
