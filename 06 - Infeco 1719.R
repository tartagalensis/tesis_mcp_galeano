...
data1<-read.csv("base_infeco_1719.csv")


dsPpre <- data1[,5:10] #De que col a que col van los rtados de las elecciones prim vuelta
dsPpre <- dsPpre/data1[,11] #n de col de electores
dsPpre <- cbind(dsPpre,1-rowSums(dsPpre))
dsPpre <- as.matrix(dsPpre)

dsGpre <- data1[,12:16] #de que col a que col van los rtados de las elecciones 2da vuelta
dsGpre <- dsGpre/data1[,11] #n de col de electores
dsGpre <- cbind(dsGpre,1-rowSums(dsGpre))
dsGpre<- as.matrix(dsGpre)

#Paso a General
newdata<-as.matrix(cbind(dsPpre,dsGpre))
estsPG<-params.estim(newdata,nR=7,nC=6) #10 filas (1 para blancos/ausentes), 3 cols (1 para blancos/ausentes)
fracsPG<-calc.fractions(estsPG,nR=7,nC=6)
colnames(fracsPG)<-colnames(dsGpre)
rownames(fracsPG)<-colnames(dsPpre)

##Datos Summary
VotosPaso <- data1[,5:10]
VotosPaso <- cbind(VotosPaso,data1[,11]-rowSums(VotosPaso))
VotosGeneral <- data1[,12:16]
VotosGeneral <- cbind(VotosGeneral ,data1[,11]-rowSums(VotosGeneral))
...
tabla %>%
  ...
  write_csv("inferenciaEcoResultados_1719.csv")
